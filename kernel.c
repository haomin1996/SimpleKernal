...
static void put_char(char c) {
  if (c == '\n')
    ...
    VIDEO[cursor++] = (uint16_t)c | 0x0F00;
...
extern void first_task_switch(uint32_t);
static void idt_set_gate(int n, void* h) {
...
static void q_push(struct queue* q, char c) {
  if (((q->t + 1) % QSIZE) != q->h) {
    q->b[q->t] = c;
    q->t = (q->t + 1) % QSIZE;
  }
}
static int q_pop(struct queue* q, char* c) {
...
static void wc_task(void);
static void task_init(struct task* t, void (*e)(void)) {
...
static void echo_task(void) {
  ...
  for (;;) {
    char c;
    if (q_pop(&tasks[0].in, &c)) {
      if (c == '\n') {
        tasks[0].line[tasks[0].pos] = '\0';
        print("\nYou typed: ");
        print(tasks[0].line);
        print("\nEcho> ");
        tasks[0].pos = 0;
        ...
        if (tasks[0].pos < sizeof(tasks[0].line) - 1) {
          tasks[0].line[tasks[0].pos++] = c;
          print((char[]){c, 0});
        }
      }
    }
...
static void wc_task(void) {
  ...
  for (;;) {
    char c;
    if (q_pop(&tasks[1].in, &c)) {
      if (c == '\n') {
        tasks[1].line[tasks[1].pos] = '\0';
        int w = count_words(tasks[1].line);
        print("\nWord count: ");
        print_dec(w);
        print("\nWordCount> ");
        tasks[1].pos = 0;
        ...
        if (tasks[1].pos < sizeof(tasks[1].line) - 1) {
          tasks[1].line[tasks[1].pos++] = c;
          print((char[]){c, 0});
        }
      }
    }
...
void kernel_main(void) {
  print("SimpleKernel Preemptive Demo\n");
  pic_remap();
  idt_set_gate(32, irq0_stub);
  idt_set_gate(33, irq1_stub);
  idt_install();
  pit_init();
  task_init(&tasks[0], echo_task);
  task_init(&tasks[1], wc_task);
  focus = 0;
  current = 0;
  first_task_switch(tasks[0].esp);
  while (1);
}
